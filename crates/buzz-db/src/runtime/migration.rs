...
mod tests {
    ...
    fn embedded_migrator_contains_consolidated_initial_schema() {
        ...
        // This fork does not include upstream's unrelated migration 0043
        // (push-gateway deployment preparation), but keeps migration 0044 as
        // the NIP-FI ledger cleanup. SQLx migrations may have version gaps.
        ...
        // ledger tables are dead code. Restore the pre-0041 exclusion body so
        // the deletion catalog no longer includes the removed relations.
        assert_eq!(migrations[42].version, 44);
        let ledger_removal = migrations[42].sql.as_str();
